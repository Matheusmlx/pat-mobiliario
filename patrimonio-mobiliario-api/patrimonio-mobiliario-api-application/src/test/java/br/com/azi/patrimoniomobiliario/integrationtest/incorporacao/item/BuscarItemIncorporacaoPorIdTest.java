...
@Rollback
@Sql("/datasets/incorporacao/item/buscar-item-incorporacao-por-id.sql")
@Transactional
public class BuscarItemIncorporacaoPorIdTest {
    ...
    public void deveRetornarItemIncorporacao() throws Exception {
        mockMvc.perform(
            get("/patrimonios/incorporacao/1/item/2")
                .headers(AuthenticationHelper.getHeaders())
                .contentType(MediaType.APPLICATION_JSON)
        )
            .andExpect(status().is2xxSuccessful())
            .andExpect(jsonPath("$.id", equalTo(2)))
            .andExpect(jsonPath("$.estadoConservacao", equalTo(2)));
    }
