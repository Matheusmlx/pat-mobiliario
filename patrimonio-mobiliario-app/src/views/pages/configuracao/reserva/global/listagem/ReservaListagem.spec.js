...
describe('ReservaListagem', () => {
    ...
    describe('Events', () => {
        ...
        it('Deve emitir o evento de acessar a reserva e redirecionar para edição se situação em elaboração', async () => {
            wrapper = shallowMount(ReservaListagem, {
                localVue,
                router,
                store,
        ...
        it('Deve emitir o evento de acessar a reserva e redirecionar para edição se situação parcial e existe intervalos em elaboração', async () => {
            wrapper = shallowMount(ReservaListagem, {
                localVue,
                router,
                store,
        ...
        it('Deve emitir o evento de acessar a reserva e redirecionar para visualização se situação parcial e todos intervalos estão finalizados', async () => {
        ...
        it('Deve emitir o evento de acessar a reserva e redirecionar para visualização se situação finalizado', async () => {
            wrapper = shallowMount(ReservaListagem, {
                localVue,
                router,
                store,
    ...
    describe('Methods', () => {
        ...
        describe('tratarEventoAcessar', () => {
            ...
            it('deve redirecionar para edição quando situação for EM_ELABORACAO', () => {
                wrapper.vm.tratarEventoAcessar({id: 1, situacao: 'EM_ELABORACAO'})
                expect(router.push).toHaveBeenCalled()
                expect(router.push.mock.calls[0][0]).toEqual({name: 'ReservaEdicao', params: {id: 1}})
            })

            it('deve redirecionar para edição quando situação for PARCIAL e houver itens em elaboração', async () => {
            ...
            it('deve redirecionar para visualização quando situação for PARCIAL e NÃO houver itens em elaboração', async () => {
                actions[actionTypes.CONFIGURACAO.RESERVA.GLOBAL.INTERVALO.EXISTE_EM_ELABORACAO] = jest.fn().mockReturnValue(false)
                wrapper = shallowMount(ReservaListagem, {
                    localVue,
                    router,
            ...
            it('deve redirecionar para visualização quando situação for FINALIZADO', async () => {
