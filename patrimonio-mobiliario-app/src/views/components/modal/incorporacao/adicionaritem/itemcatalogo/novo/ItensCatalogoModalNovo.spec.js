...
import ItensModal from './ItensCatalogoModalNovo'
import applicationTestBuilder from '@/application/ApplicationTestBuilder'
import {actionTypes, mutationTypes} from '@/core/constants'

describe('ItensModal', () => {
    let wrapper, localVue,router, state, mutations, actions, vuetify, $emit = jest.fn()
    ...
    describe('Methods', () => {
        ...
        it('Deve remover campo', async () => {
            wrapper = defaultMount()

            await flushPromises()
            expect(wrapper.vm.itens.length).toEqual(3)
            wrapper.vm.removerCampo()

            expect(wrapper.vm.itens.length).toEqual(2)
        })
    })
})
